...
def train(ID_START, id_and_X, RESULT, model):
    ...
    for index,row in id_and_X.iterrows():
        ...
        if(id_now != row['crsp_fundno'] or index == id_and_X.shape[0]-1 ):
            #print("-------------------------------------------------------------------")
            ...
            result_series = pd.Series({'Beta1':beta_now[0],'Beta2':beta_now[1],'Beta3':beta_now[2],\
                'Beta4':beta_now[3],'Beta5':beta_now[4],'Beta6':beta_now[5],'Beta7':beta_now[6],\
                    'Beta8':beta_now[7],'crsp_fundno':id_now})
            #print(result_series)
    ...
    RESULT.insert(0,'crsp_fundno',RESULT.pop('crsp_fundno'))
...
def main():
    input= os.listdir('D:\\python_workspace\\kdd-hw\\linerlog\\processed_Daily beta\\')
    #print(input)
    for data_name in input:
        data = pd.read_csv('D:\\python_workspace\\kdd-hw\\linerlog\\processed_Daily beta\\'+data_name)
        #print(data.head())
        id_and_features = ['crsp_fundno','Return','F1','F2','F3','F4','F5','F6','F7','F8']
        id_and_X = data[id_and_features]

        RESULT = pd.DataFrame(columns=['Beta1','Beta2','Beta3','Beta4','Beta5','Beta6','Beta7','Beta8','crsp_fundno'])
        ID_START = id_and_X.loc[0][0]
        train(ID_START, id_and_X, RESULT, model)
        RESULT.to_csv('D:\\python_workspace\\kdd-hw\\linerlog\\result_monthly\\'+data_name,index=False)
